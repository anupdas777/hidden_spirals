...
required_vars = { ...
    ...
    'evolve_dt', ...
    'convergence_tol', ...
    'lambda_zero_cleanup_steps', ...
    'homotopy_perturb_sigma', ...
    'checkerboard_tol', ...
    'continuation_full_zero_tol', ...
    'lambda_start', ...
    'lambda_end', ...
    'ds_init', ...
    'ds_min', ...
    ...
    'pos_tol', ...
    'zero_tol', ...
    'expected_neutral_max'};

missing_vars = required_vars(cellfun(@(v) ~(exist(v, 'var') == 1), required_vars));
if ~isempty(missing_vars)
    error('Missing configuration variable(s): %s', strjoin(missing_vars, ', '));
end
...
if exist('trial_selection', 'var') && ~isempty(trial_selection)
    trial_selection = unique(trial_selection(:)');
    trial_selection = trial_selection(trial_selection >= 1 & trial_selection <= num_trials);
    trial_indices = trial_selection;
else
...
for idx = 1:num_to_process
    ...
    if result.method > 0
        f_interp = figure('Visible', 'off');
        imagesc(result.Target_temp); colormap('hsv'); clim([0 2*pi]); colorbar;
        axis square;
        title(sprintf('Trial %d, Interpolated 29x29', tri_num));
        print(f_interp, fullfile(directory, sprintf('trial_%d_interpolated_phase_29.jpg', tri_num)), '-djpeg', '-r150');
        close(f_interp);
