...
def load_subj_events(task, subject, montage, as_df=False, remove_no_eeg=True):
    ...
    if task in r1_data.experiment.unique():
        ...
        # load all and concat
        events = pd.concat([CMLReader(subject=subject, experiment=task, session=session).load('events') for session in sessions])
...
def load_elec_info(subject, montage=0, bipolar=True):
    ...
    # if not in r1 protocol, annoying, there are multiple possible locations for matlab data
        ...
        # Option 2: there is no subject specific file, look in the older aggregate file. Lot's of steps.
            ...
            e_type[np.in1d(elec_df.montage, ['hipp', 'inf'])] = ''
            elec_df['type'] = e_type
...
def load_eeg(events, rel_start_ms, rel_stop_ms, buf_ms=0, elec_scheme=None, noise_freq=[58., 62.],
             resample_freq=None, pass_band=None, use_mirror_buf=False, demean=True, do_average_ref=False):
    """
    ...
    demean: bool
        If True, will subtract the mean voltage between rel_start_ms and rel_stop_ms from each channel
    do_average_ref: bool
    ...
    # resample if desired. Note: can be a bit slow especially if have a lot of eeg data
        ...
        #eeg = ResampleFilter(eeg, resample_freq).filter()
        #Anup changed here
        #for this_chan in range(eeg.shape[1]):
...
def compute_power(events, freqs, wave_num, rel_start_ms, rel_stop_ms, buf_ms=1000, elec_scheme=None,
    ...
    if do_average_ref:
        ...
        eeg_all_chans = load_eeg(events, rel_start_ms, rel_stop_ms, buf_ms=buf_ms, elec_scheme=elec_scheme,
                                 noise_freq=noise_freq, resample_freq=None, use_mirror_buf=use_mirror_buf,
                                 do_average_ref=do_average_ref) 
        eeg_all_chans = ResampleFilter(eeg_all_chans, resample_freq).filter()
    else:
        eeg_all_chans = None
